#ifndef COHORT_EXP_SMALLEST_THREE_H
#define COHORT_EXP_SMALLEST_THREE_H
...
class Experiment_Smallest_Three : public Experiment{
    public:
        // Readability aliases
        using this_t = Experiment_Smallest_Three;
    ...
    public:
        Experiment_Smallest_Three();
        ~Experiment_Smallest_Three();
};

Experiment_Smallest_Three::Experiment_Smallest_Three():
...
void Experiment_Smallest_Three::SetupProblem(){
    std::cout << "Setting up problem: \"smallest three\"" << std::endl;
...
void Experiment_Smallest_Three::SetupSingle(org_t& org, const input_t& input){
    ...
    emp_assert(hardware->GetMemSize() >= 3, "Smallest Three requires a memory size of at least 3");
    submitted = false;
    submitted_val = 0;
    // Configure inputs.
...
void Experiment_Smallest_Three::SetupDilution(){
    if(!problems_loaded){
...
void Experiment_Smallest_Three::SetupSingleTest(org_t& org, size_t test_id){
    input_t & input = training_set.GetInput(test_id);
    cur_test_id = test_id;
    SetupSingle(org, input);
}

void Experiment_Smallest_Three::SetupSingleValidation(org_t& org, size_t test_id){
    input_t & input = test_set.GetInput(test_id);
    cur_test_id = test_id;
    SetupSingle(org, input);
}

void Experiment_Smallest_Three::RunSingleTest(org_t& org, size_t test_id, size_t local_test_id){
...
Experiment::TestResult Experiment_Smallest_Three::RunSingleValidation(org_t& org, size_t test_id){
    // Check for auto-pass cases
    if(test_set.GetInput(test_id).second){
        std::cout << "Diluted TEST (validation) case found! This is an error!" << std::endl;
        exit(-1);
...
std::pair<Experiment_Smallest_Three::input_t, Experiment_Smallest_Three::output_t> 
            Experiment_Smallest_Three::LoadTestCaseFromLine(const emp::vector<std::string> & line) {
    input_t input;   
    output_t output; 
    // Load input.
...
Experiment_Smallest_Three::output_t Experiment_Smallest_Three::GenCorrectOutput(input_t &input){
    return *std::min_element(input.first.begin(), input.first.end());
}

void Experiment_Smallest_Three::SetupInstructions(){
...
void Experiment_Smallest_Three::Inst_LoadNum1(hardware_t & hw, const inst_t & inst) {
...
void Experiment_Smallest_Three::Inst_LoadNum2(hardware_t & hw, const inst_t & inst) {
    hardware_t::CallState & state = hw.GetCurCallState();
    hardware_t::Memory & wmem = state.GetWorkingMem();
...
void Experiment_Smallest_Three::Inst_LoadNum3(hardware_t & hw, const inst_t & inst) {
    hardware_t::CallState & state = hw.GetCurCallState();
    hardware_t::Memory & wmem = state.GetWorkingMem();
...
void Experiment_Smallest_Three::Inst_SubmitNum(hardware_t & hw, const inst_t & inst) {
    hardware_t::CallState & state = hw.GetCurCallState();
    hardware_t::Memory & wmem = state.GetWorkingMem();
