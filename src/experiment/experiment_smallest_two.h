#ifndef COHORT_EXP_SMALLEST_TWO_H
#define COHORT_EXP_SMALLEST_TWO_H
...
class Experiment_Smallest_Two : public Experiment{
    public:
        // Readability aliases
        using this_t = Experiment_Smallest_Two;
        using input_t = std::pair<std::array<int, 2>, bool>; // bool is for auto-pass
    ...
    public:
        Experiment_Smallest_Two();
        ~Experiment_Smallest_Two();
};

Experiment_Smallest_Two::Experiment_Smallest_Two():
...
void Experiment_Smallest_Two::SetupProblem(){
    std::cout << "Setting up problem: \"smallest two\"" << std::endl;
...
void Experiment_Smallest_Two::SetupSingle(org_t& org, const input_t& input){
    ...
    emp_assert(hardware->GetMemSize() >= 2, "Smallest Two requires a memory size of at least 2");
    submitted = false;
    submitted_val = 0;
    // Configure inputs.
...
void Experiment_Smallest_Two::SetupDilution(){
    if(!problems_loaded){
...
void Experiment_Smallest_Two::SetupSingleTest(org_t& org, size_t test_id){
    input_t & input = training_set.GetInput(test_id);
    cur_test_id = test_id;
    SetupSingle(org, input);
}

void Experiment_Smallest_Two::SetupSingleValidation(org_t& org, size_t test_id){
    input_t & input = test_set.GetInput(test_id);
    cur_test_id = test_id;
    SetupSingle(org, input);
}

void Experiment_Smallest_Two::RunSingleTest(org_t& org, size_t test_id, size_t local_test_id){
...
Experiment::TestResult Experiment_Smallest_Two::RunSingleValidation(org_t& org, size_t test_id){
    // Check for auto-pass cases
...
std::pair<Experiment_Smallest_Two::input_t, Experiment_Smallest_Two::output_t> 
            Experiment_Smallest_Two::LoadTestCaseFromLine(const emp::vector<std::string> & line) {
    input_t input;   
    output_t output; 
    // Load input.
    ...
    input.second = false;
    // Load output.
    output = std::atof(line[2].c_str());
    emp_assert(output == GenCorrectOutput(input));
    return {input, output};
}

Experiment_Smallest_Two::output_t Experiment_Smallest_Two::GenCorrectOutput(input_t &input){
    return *std::min_element(input.first.begin(), input.first.end());
}

void Experiment_Smallest_Two::SetupInstructions(){
...
void Experiment_Smallest_Two::Inst_LoadNum1(hardware_t & hw, const inst_t & inst) {
...
void Experiment_Smallest_Two::Inst_LoadNum2(hardware_t & hw, const inst_t & inst) {
    hardware_t::CallState & state = hw.GetCurCallState();
    hardware_t::Memory & wmem = state.GetWorkingMem();
...
void Experiment_Smallest_Two::Inst_SubmitNum(hardware_t & hw, const inst_t & inst) {
    hardware_t::CallState & state = hw.GetCurCallState();
    hardware_t::Memory & wmem = state.GetWorkingMem();
