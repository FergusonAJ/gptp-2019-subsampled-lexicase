...
org_gen_col_names = "num_taxa,total_orgs,ave_depth,num_roots,mrca_depth,diversity,mrca_changes,mean_evolutionary_distinctiveness,min_evolutionary_distinctiveness,max_evolutionary_distinctiveness,variance_evolutionary_distinctiveness,mean_pairwise_distance,min_pairwise_distance,max_pairwise_distance,variance_pairwise_distance,current_phylogenetic_diversity,tree_size,num_sparse_taxa,mean_sparse_pairwise_distances,sum_sparse_pairwise_distances,variance_sparse_pairwise_distances" 
org_gen_num_cols = len(org_gen_col_names.split(','))
# Get ALL the relevant data
with open('diversity_data.csv', 'w') as fp:
    fp.write('problem' \
             + ',' \
             + 'treatment' \
             + ',' \
            ...
            for size in sizes:
                for dil in dilutions:
                    ...
                    for replicate in range(0, num_replicates):
                        ...
                        behavioral_diversity = 'NA'
                        unique_behavioral_phenotypes = 'NA'
                        if solution_found: 
                            with open(scratch_dir+replicate_dir+'phenotypic_diversity.csv') as pd_fp:
                                header = pd_fp.readline()
                                for line in pd_fp:
                                    line = line.strip()
                                    if line != '':
                                        tmp_L = line.split(',')
                                        behavioral_diversity = float(tmp_L[3])
                                        unique_behavioral_phenotypes = float(tmp_L[4])
                        # Organism Genotypic Stats
                        org_gen_vals = "NA," * org_gen_num_cols
                        if solution_found: 
                            with open(scratch_dir + replicate_dir + 'org_gen_sys.csv') as og_fp:
                                header = og_fp.readline()
                                for line in og_fp:
                                    line = line.strip()
                                    if line != '':
                                        tmp_L = line.split(',')
                                        org_gen_vals = ''
                                        for i in range(1, len(tmp_L) - 2):
                                            org_gen_vals += tmp_L[i] + ','
                        org_gen_vals  = org_gen_vals[:-1] # Trim off the final comma
                        ...
                        fp.write(prob.name \
                                 ...
                                 + replicate_dir \
                                 + ',' \
                                 + str(behavioral_diversity) \
                                 + ',' \
                                 + str(unique_behavioral_phenotypes) \
                                 + ',' \
                                 + org_gen_vals \
                                 + '\n')
                    print(prob.name + ' ' \
