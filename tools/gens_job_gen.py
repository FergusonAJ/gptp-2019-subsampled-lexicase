...
# Treatments
treatments = [trt_cohort, trt_downsampled, trt_full]#, trt_truncated]

# Cohort (or equiv.) size
sizes = [size_100_300, size_50_300, size_25_300, size_10_300, size_5_300]

# Dilution rates
dilutions = [dil_0_0]

seed_start_offset = 70000
...
def write_job_file(prob, trt, size, dil, out_dir = None, extra_seed = None, \
    ...
    prog_cohort_size = size.prog_cohort_size
    if trt.name == 'full':
        num_tests = 100
        prog_cohort_size = 1000   
    with open(filename, 'w') as fp:
        ...
        fp.write('PROG_COHORT_SIZE=' + str(prog_cohort_size) + '\n')
        fp.write('NUM_TESTS=' + str(int(prob.test_case_factor * num_tests))  + ' ' + \
...
if __name__ == '__main__':        
    ...
    for prob in problems:
        ...
        for trt in treatments:
            for size in sizes:
                for dil in dilutions:
                    write_job_file(prob, trt, size, dil, hours = 18)
